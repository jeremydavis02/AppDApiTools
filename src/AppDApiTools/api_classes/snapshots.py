...
class Snapshots(ApiBase):
    ...
    def get_function_parms(cls, subparser):
        ...
        class_commands = subparser.add_parser('Snapshots', help='Snapshots commands')
    ...
    def get_app(self):
        self.do_verbose_print('Doing Snapshots Get...')

        if self.args.name is None and self.args.id is None:
            print(f'Snapshots get requires --name or --id, see --help')
            sys.exit()
